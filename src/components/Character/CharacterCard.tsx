...
const CharacterCard: React.FC<CharacterProps> = ({ className, level, name, index, selected, toggleDialog }) => {
    ...
    return (
        <div className={`relative cursor-pointer border-accent-grey border-2 shadow-lg flex items-center w-40 h-20 rounded-md hover:ring-2 focus:ring-gray-300 ${selected ? "ring ring-gray-300 ring-offset-1" : ""}`} onClick={() => dispatch(setCharIndex(index))}>
